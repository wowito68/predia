// ============================================
// app/api/pacientes/[id]/eliminar-completo-prisma/route.ts
// ============================================
// VERSIÓN ALTERNATIVA: Similar a eliminar-completo, mantiene consistencia
...
/**
 * DELETE - Eliminar paciente completamente (hard delete alternativo)
 *
 * Requiere:
 * - rol = 1 (Administrador)
 ...
 */
export const DELETE = requireAuth(async (request: NextRequest, context: { params?: any; user: JWTPayload }): Promise<Response> => {
  try {
    // ========================================
    ...
    const resultado = await transaction(async (connection: PoolConnection) => {
    ...
    console.log(`✅ Eliminación Alternativa - ID: ${id_paciente}, Cédula: ${resultado.paciente.cedula}`)

    return NextResponse.json(
      {
        success: true,
        message: "Paciente eliminado completamente con todas sus relaciones (versión alternativa)",
        data: {
    ...
    console.error("❌ Error en eliminar-completo-prisma:", {
      error: errorMessage,
